...
public final class VidScreenFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(VidScreenPayload.CHANNEL,
                (client, handler, buffer, responseSender) -> {
                    final byte[] data;
                    try {
                        data = VidScreenPayload.read(buffer).data();
                    } catch (RuntimeException error) {
                        LOGGER.warn("Rejected malformed VidScreen payload: {}", error.getMessage());
                        return;
                    }
                    client.execute(() -> accept(data));
                });
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> join());
    ...
    private static void join() {
        ...
        try {
            send(new ClientHello(ProtocolVersion.MAJOR, ProtocolVersion.MINOR, "0.1.0", "fabric",
        ...
        try {
            ...
            if (message instanceof ServerHello && !((ServerHello) message).accepted()) {
                ...
                if (message instanceof ClockResponse) {
                    ClockResponse clock = (ClockResponse) message;
                    CLOCK.add(new ClockSample(clock.clientSendTimeMillis(), clock.serverReceiveTimeMillis(),
                            clock.serverSendTimeMillis(), System.currentTimeMillis()));
                }
    ...
    private static void tick() {
        ...
        if (now < nextClockSampleMillis || !ClientPlayNetworking.canSend(VidScreenPayload.CHANNEL)) {
            return;
        }
    ...
    private static void send(WireMessage message) throws ProtocolException {
        ClientPlayNetworking.send(VidScreenPayload.CHANNEL, new VidScreenPayload(CODEC.encode(message)).toBuffer());
    }
}
