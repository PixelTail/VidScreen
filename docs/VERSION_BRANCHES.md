...
The GitHub repository is `https://github.com/PixelTail/VidScreen`. The independent `version/1.21.1` experimental branch is initialized alongside the modern branches; older branches are added incrementally because their Gradle, mappings, loader, rendering, networking, and Java constraints are not interchangeable with 26.x.
