...
- the 1.18.2 Java 17 feasibility lane has isolated Gradle builds for Fabric, Forge, and Paper with bounded legacy transport adapters; its exact local builds pass with Gradle 8.6 for Fabric/Paper and Gradle 7.6.4 for Forge, while branch CI and all runtime gates remain pending, so this is not a support claim.
